...
func (p *commandCodeProvider) buildHeaders(apiKey string) map[string]string {
...
// ChatCompletion performs a (logically) non-streaming chat completion. Command Code
// always streams upstream, so the SSE body is fully read and aggregated into one response.
func (p *commandCodeProvider) ChatCompletion(ctx *schemas.BifrostContext, key schemas.Key, request *schemas.BifrostChatRequest) (*schemas.BifrostChatResponse, *schemas.BifrostError) {
	...
	jsonBody, marshalErr := sonic.Marshal(buildCommandCodeRequest(request))
	if marshalErr != nil {
	...
	response, aggErr := aggregateCommandCodeStream(body, request.Model)
	if aggErr != nil {
...
// ChatCompletionStream performs a streaming chat completion against Command Code.
func (p *commandCodeProvider) ChatCompletionStream(ctx *schemas.BifrostContext, postHookRunner schemas.PostHookRunner, postHookSpanFinalizer func(context.Context), key schemas.Key, request *schemas.BifrostChatRequest) (chan *schemas.BifrostStreamChunk, *schemas.BifrostError) {
	apiKey := key.Value.GetValue()
	if apiKey == "" {
		return nil, providerUtils.NewBifrostOperationError("Command Code API key required", fmt.Errorf("missing api key"))
	}
	...
	// When the request arrived via the Responses API (and was redirected to chat
	// streaming), convert each chat chunk into Responses stream events so clients
	// hitting /v1/responses get response.* events, not chat.completion.chunk.
	...
	go func() {
		...
		// emitChat sends one chat chunk to the client. In Responses-fallback mode it
		// converts the chunk into Responses stream events first. Returns true if the
		// stream must abort (an error event was emitted).
		emitChat := func(resp *schemas.BifrostChatResponse) bool {
			if !isResponsesFallback {
				providerUtils.ProcessAndSendResponse(ctx, postHookRunner, providerUtils.GetBifrostResponseForStreamResponse(nil, resp, nil, nil, nil, nil), responseChan, postHookSpanFinalizer)
				return false
			}
			if resp.Usage != nil {
				usageSeen = true
		...
		if isResponsesFallback {
			...
			// the closing responses events (output_item.done, response.completed).
...
// Responses maps to ChatCompletion (Command Code has no native Responses API).
...
// ResponsesStream maps to ChatCompletionStream.
